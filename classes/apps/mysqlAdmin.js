...
class main { 
	initClass() {
		@topTitle=this.getWidget('canvas', true)
		@splitter=this.getWidget('splitter', true)
		@left=page('leftPanel')
		@tab=left.getWidget('leftTab')
		@dbTableInfo=this.getWidget('dbTableInfo')
		@dbApiInfo=this.getWidget('dbApiInfo') 
		@dbTableForm=dbTableInfo.getWidget('form') class(dbTableForm)
	...
	initContent() { 
		...
		if(page) {
			...
			editor=page.makeWidget('editor','e0')
			form=page.makeWidget('canvas','c0')
			form.minSize(0,30)
...
class dbTableForm & form {
	initClass() {
		@tree=this.makeWidget('tree','dbTableTree', 'rcBody') class(tree,'tree')
		@combo=this.makeWidget('combo','dbTableFilterKind', 'rcCombo')
		@filter=this.makeWidget('input','dbTableFilter', 'rcFilter')
		@popup=this.makeWidget('canvas','popupTableInfo') class(popup, 'popupTableInfo', true)
		popup.parentWidget(null)
		popup.flags('tool', true)
		
		this.initFormCheck(parent)
		...
		this.setEvent('onMouseClick', this.formClick)
	...
	updateForm(rc) {
		...
		hbox(rcBar.margin(2,2,0,4),"85,110,*").inject(rcCombo, rcFilter, rcSpace)
		rcBox=rcBar.rightCenter(80, 24, -5)
		arr=this.addArray("@rectButtons", true)
		while(rcCur, hbox(rcBox, 3), idx ) {
			arr.add(rcCur)
		}
		this.setFormRect()
	...
	drawForm(dc, rc) {
		...
		buttons=this.addArray("@rectButtons")
		while(rcCur, buttons, idx ) {
			rcBtn=rcCur.incrXW(2,2)
			...
			switch(idx) {
			case 0:
				dc.image(rcBtn.center(18,18), 'vicon:database_edit')
			case 1:
				dc.image(rcBtn.center(18,18), 'vicon:database_table')
			case 2:
				icon=when(this.expandFlag,'icons:arrow_up','icons:arrow_down')
				dc.image(rcBtn.center(18,18), icon)
			default:
			}
		}
	}
	formMouseMove(pt) {
		_tip=func(idx) {
			switch(idx) {
			case 0: return '테이블 정보 추가/수정';
			case 1: return '테이블 정보보기';
			case 2: return '테이릅 트리 펼치기';
			default:
			}
		};
		arr=this.addArray("@rectButtons")
		while(rc, arr, idx) {
			if(rc.contains(pt)) {
				not(rc.eq(this.overRect) ) this.redraw()
				this.overRect = rc
				return this.tooltip(_tip(idx), true)
			}
		}
		if(this.overRect) {
			this.tooltip('',false)
	...
	formClick(pt) {
		...
		while(rc, arr, idx) {
			...
			case 0:
				...
				while(cur, root) {
					cur.type='table'
					db.fetchAll(this.conf('columnInfo'), cur)
					while(sub,cur) sub.type='column'
				}
				tree.update()
			case 1:
				alert('테이블 정보조회기능 구현중')
			case 2:
				expand=this.expandFlag
				this.expandFlag=when(expand, false, true)
				page('main').treeExpandAll(this.expandFlag, true)
				this.redraw()
			}
		}
	}
...
class dbApiForm & form {
	initForm(parent) {
		@tree=this.makeWidget('tree','dbApiTree', 'rcBody') class(tree,'tree')
		@combo=this.makeWidget('combo','dbApiFilterKind', 'rcCombo')
		@filter=this.makeWidget('input','dbApiFilter', 'rcFilter')
		this.initFormCheck(parent)
...
class formSqlEditForm {
	initClass() {
		class(this, 'form')
		@grid=this.makeWidget('grid','gridDbSelect','rcGrid')
		grid.model('name')
...
class appLangForm {
	initClass() {
		class(this,'form')
		@db=Baro.db('muk')
		@grid=this.makeWidget('grid', 'gridTest', 'rcGrid') class(grid, 'grid')
		input=this.makeWidget('input', 'inlineEditor')
		input.parentWidget(this)
		input.setEvent('onKeyDown', this, this.keydown)
		grid.setInput(input)
		...
		@comboUseYn=this.makeWidget('combo','langUseYn', 'rcComboUseYn')
		@comboKind=this.makeWidget('combo','langFilterKind', 'rcComboKind')
		@filter=this.makeWidget('input','langFilter','rcFilter')
