...
dbUtil.rootNode() { return dataNode; }
...
ClassComboSelect.initClassCombo(cls, all) {	str=Cf.info('funcVar', cls, 'member').str();
...
ClassFuncGrid.okClick() {	root=grid.rootNode();
...
fc(root);}
ClassFuncGrid.gridDoubleClick(node) {	root=grid.rootNode();
classNode=root.currentClass;
if( classNode[@classBase] ) {
	table="class_func";
	root[inherit]="${classNode[@classBase]}.${classNode[@className]}";
...
ClassFuncsManager.initPage(root, append) {		if( root ) {
		@currentNode=root;
	}
...
ClassFuncsManager.saveSrc() {	me=this;
_save=func(&s) {
	map=_node(cf,'ClassFuncsMap');
	root=currentNode;
	classNode=null;
...
page.save.disable();}
ClassFuncsManager.runSrc(flag) {	me=this;
_run=func(&s) {
	map=_node(cf,'ClassFuncsMap');
	err='';
	root=currentNode; 
