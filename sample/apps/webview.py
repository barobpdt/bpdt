...
class Bridge(QObject):
    ...
    def logAppend(self, msg):
        self.webview.logAppend(f'bridge:{msg}')
...
<script>
	...
	function sendMessage() {
		bridge.logAppend("안녕하세요! JS에서 보냄");
	}
</script>
'''
...
class WebView(QWebEngineView):
	...
	def onUrlChange(self, url):
		if url.scheme() == "my":
			print(f"[urlChanged] my:// 차단: {url.toString()}")
			self.back()
	...
	def acceptNavigationRequest(self, url,  _type, isMainFrame):
		...
		self.logAppend(f'changeUrl:{url.toString()}')
		return False  # 내비게이션 차단
