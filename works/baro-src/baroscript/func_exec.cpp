...
XListArr* getParamArray(XFunc* func, XFuncNode* fn, StrVar* rst ) {
    ...
    XListArr* list = getLocalArray(true);
    for( int n=0;n<cnt;n++ ) {
        XFunc* param = func->xparams.getvalue(n);
        if( param->xftype==FTYPE_OPER ) {
            break;
        }
        execParamFunc(param, fn, list->add() );
    }
    return list;
}
U32 makeLocalFunction( XFunc* func, bool bset, XFuncNode* fn, StrVar* rst, LPCC name ) {
    ...
    if( bset) {
        qDebug("make local function bset not define !!!");
        return 0;
    }
    XFunc* pfc=NULL;
    // LPCC fcode=NULL;
			...
			if( ch=='0' && pv.CharAt(sp+1)=='x' ) {
                ...
                if( ch=='(' || ch=='.' ) {
                    if( ch=='.' ) {
						...
						qDebug("xxxxxxxxxxxxxxxxxxxxxx ch=%c ", ch);
                        if( ch=='(') {
                            fchk=true;
                        } else {
				...
				if( isnum(val) ) { // ||
                    rst->add(val);
				} else if(ccmpl("and",3,val) || ccmpl("or",2,val) ) {
					rst->add(' ').add(val).add(' ');
                } else {
                    LPCC extr=NULL;
                    if( ch=='.' ) {
                    ...
                    // fst = false;
					StrVar* sv = getFuncVar(fn, val);
					if(SVCHK('n',0) && slen(extr)>0 ) {
						TreeNode* cur=(TreeNode*)SVO;
						int pos = IndexOf(extr,'.');
						if( pos>0 ) {
							char* code=(char*)extr;
							extr=extr+pos+1;
							code[pos]=0;
...
int execFunc(XFunc* func, XFuncNode* fn, StrVar* rst, StrVar* cur ) {
    ...
    switch( func->getType() ) {
        ...
        case FTYPE_CALLBACK: {
            ...
            makeLocalFunction(func, false, fn, rst, NULL );
        } break;
        case FTYPE_TEMPLATE: {
        } break;
        case FTYPE_CONST: {
        ...
        case FTYPE_HASH: {
            ...
            if( func->xpv.ch()=='[' ) {
                ...
                h=root->addNode(FMT("node%d",func->xfuncRef), true);
        ...
        case FTYPE_FUNC: {
            ...
            if( nchk ) {
                ...
                if( cc || (func->xdotPos>0 && nchk==1) ) {
                    ...
                    bool bRef=false;
                    if( cc==false ) {                        
                        func->getVarBuf(varName,64);
                        if(slen(varName)) varNameCode=varName;
                        if( varNameCode[0]=='&' ) {
                            varNameCode+=1;
                            if( varNameCode[0]=='@' ) {
                    ...
                    if( cur ) {
                        ...
                        if( cur->charAt(0)=='\b') {
                            ...
                            if( func->xfuncRef ) { // && (func->xdotPos==0 || func->xfflag&FFLAG_STRSET) func->xfflag==0 ||
                                ...
                                qDebug("#9#[%s] function not define (function:%s])\n", getBaseFuncName(fn), func->getFuncName());
                                berror=true;
                            }
                        }
                    } else if( varNameCode ) {
                        ...
                        qDebug("#0#[%s] object function not define (name: %s.%s) \n", getBaseFuncName(fn), func->getVarName(), funcName );
                        berror=true;
                    }
                    ...
                    */
                    ...
                    if( cur ) {
                        ...
                        if( !funcCall( getFuncVar(fn,funcNm), fn, arrs, rst) ) {
                            if(arrs==NULL) arrs=getLocalArray(true);
                ...
                if( arrs && arrs->isNodeFlag(FLAG_LOCALARRAY_SET) ) {
                    qDebug("==== function argument release check =======");
                    releaseLocalArray(arrs);
                }
                /*
                if( berror ) {
            ...
            if( left && func->xpv.MatchWordPos("[","]",FIND_SKIP) ) {
                ...
                if(slen(left)==1) {
                    if( ccmp(left,"#") ) {
						...
						TreeNode* curNode=SVCHK('n',0)?(TreeNode*)SVO:&gExprNode;
						parseArrayVar(sub, curNode, fn, rst->reuse(), 0x400, func );
					} else if( ccmp(left,"V") ) {
                        rst->add(func->xpv.GetVar(), func->xpv.prev, func->xpv.cur);
                        bok=false;
					} else if( ccmp(left,"E") ) {
						XParseVar sub( func->xpv.GetVar(), func->xpv.prev, func->xpv.cur );
						makeExprData(sub, fn, rst, NULL);
					} else if( ccmp(left,"X") ) {
                        XParseVar sub( func->xpv.GetVar(), func->xpv.prev, func->xpv.cur );
                        ...
                        if(ssize==1) {
                            XFunc* fcCur = func->xsubs.getvalue(0);
                            if( rst->charAt(0)=='\b') {
                                StrVar* var=getStrVarTemp();
                                PARR arrs=getParamArray(fcCur,fn,rst);
                                var->add(rst);
                                execObjectFunc(fcCur, arrs, fn, var, rst->reuse() );
                                releaseLocalArray(arrs);
                            }
                        }
					} else if( ccmp(left,"R") ) {
...
int execFuncSet(XFunc* func, XFuncNode* fn, StrVar* origin ) {
        ...
        if( code[0]=='@') {
            ...
            if(SVCHK('n',0)) {
                ...
                if(SVCHK('f',0) ) {
                    ...
                    if( fnInit==fn ) {
                        qDebug("#0# execFuncSet fnInit==fn");
                    }
                    sv=fnInit->GetVar(code+1);
                    if(SVCHK('n',0) || SVCHK('a',0) ) {
                        qDebug("#0#this object member already set (%s var name error)", code);
                        return 1;
                    }
                    rst=sv;
                } else {
                    XFuncSrc* fsrc=getFuncSrc();
                    XFuncNode* fnInit=gfns.getFuncNode(fsrc->xfunc);
                    fnInit->GetVar("@this")->setVar('n',0,(LPVOID)thisNode);
                    fnInit->setNodeFlag(FLAG_PERSIST);
                    thisNode->GetVar("onInit")->setVar('f',0,(LPVOID)fnInit);
                    rst=fnInit->GetVar(code+1);
                }
            } else {
                qDebug("#0#this object member not defined (%s var name error)", code);
            }
        } else {
            rst=fn->GetVar(code);
        }
        ...
        if( code[0]=='@') {
            ...
            if(SVCHK('n',0)) {
                ...
                if(SVCHK('f',0) ) {
                    ...
                    sv=fnInit->gv(code+1);
                    if( !SVCHK('n',0)) {
                        qDebug("#0#this object member node (%s var name %s)", code, func->getFuncName() );
                        return 1;
                    }
                    rst=sv;
                } else {
                ...
                qDebug("#0#this object not defined (%s var name %s)", code, func->getFuncName() );
            }
        } else {
            rst=fn->gv(code);
        }
    ...
    if(rst==NULL ) {
        qDebug("set error %s var is null", code);
        return 1;
    }
                ...
                if( pos>0) {
                    ...
                    for( int n=0;n<8;n++ ) {
                        ...
                        if( SVCHK('n',0) ) {
                            ...
                            qDebug("#0#[%s] set node value error (var: %s, code:%s)\n",
                                   getBaseFuncName(fn), fnm, cd);
                            return 1;
                        }
                        fnm+=pos+1;
                        pos=IndexOf(fnm,'.');
    ...
    if( func->getType()==FTYPE_SET ) {
        ...
        U16 ty = func->getType();
        // qDebug("xxx func set xxx %s %d", code, ty);
        ...
        } else if( (rst->charAt(0)=='\b' && rst->charAt(1)=='1') || (origin->charAt(0)=='\b' && origin->charAt(1)=='1') ) {
            UL64 a=toUL64(rst), b=toUL64(origin);
            switch(ty) {
            case FTYPE_PLUS: { rst->setVar('1',0).addUL64(a+b); } break;
            case FTYPE_MINUS: { rst->setVar('1',0).addUL64(a-b); } break;
            case FTYPE_MULTI: { rst->setVar('1',0).addUL64(a*b); } break;
            case FTYPE_DIV: { rst->setVar('1',0).addUL64(b!=0? a/b:0); } break;
            case FTYPE_MOD: { rst->setVar('1',0).addUL64(a%b); } break;
            }
        } else {
            if( origin->charAt(0)=='\b' && origin->charAt(1)=='4' ) {
                double a=toDouble(rst);
                double c=toDouble(origin);
                switch( ty ) {
                case FTYPE_PLUS:	{ rst->setVar('4',0).addDouble(a+c); } break;
                case FTYPE_MINUS:	{ rst->setVar('4',0).addDouble(a-c); } break;
                case FTYPE_DIV:		{ rst->setVar('4',0).addDouble(c? (double)(a/c):0); } break;
...
int execCallFunc(XFunc* func, XFuncNode* fn, StrVar* rst) {
    ...
    for( int n=0;n<cnt;n++ ) {
        ...
        execParamFunc(param, fn, arrs->add());
    }
    if( arrs==NULL ) {
        qDebug("execCallFunc call error (params is null)\n");
        return 0;
    }
    sv=arrs->get(0);
    if(sv==NULL || SVCHK('9',0) || sv->length()==0 ) {
        qDebug("execCallFunc call error (params not defined)\n");
        return 0;
    }
    if( cnt>1 ) {
        char ch=sv->charAt(0);
        if( ch!='\b' || sv->charAt(1)=='s' ) {
            char* funcNm=(char*)toString(sv);
    ...
    if( SVCHK('n',0) ) {
        ...
        if( SVCHK('f',1) ) {
            ...
            funcName=toString(sv);
            sv=targetNode->gv(funcName);
            if( SVCHK('f',1) ) {
                fsrc=(XFuncSrc*)SVO;
            }
        }
        ...
        if( fsrc ) {
            ...
            setFuncSrcParam(fnCur,arrs,fsrc,2,fn);
            fnCur->GetVar("@this")->setVar('n',0,(LPVOID)targetNode);
            if( slen(funcName) ) {
                getCf()->set("@currentFuncName", funcName);
            }
            fnCur->call(fcCur,rst->reuse());
            funcNodeDelete(fnCur);
        } else {
            qDebug("execCallFunc node function not defined !!!\n");
        }
        releaseLocalArray(arrs);
        return 0;
    }
    if( SVCHK('f',1) ) {
        ...
        XFunc* fcCur=fsrc->xfunc;
        getCf()->set("@currentFuncName", "inlineFunction");
        if( arrs->size() == 1 ) {
            ...
            if( fn->isNodeFlag(FLAG_PERSIST)) {
                fnCallback=gfns.getFuncNode(fcCur,fn);
            } else {
                XFuncNode* pfn=NULL;
                if( thisNode ) {
                    sv=thisNode->gv("onInit");
                ...
                fnCallback=gfns.getFuncNode(fcCur,pfn);
                for(WBoxNode* bn=fn->First(); bn ; bn=fn->Next() ) {
                    LPCC varNm=fn->getCurCode();
                    if( varNm[0]=='@' ) continue;
                    fnCallback->GetVar(varNm)->set(bn->value );
                }
            }
            if( thisNode ) {
        ...
        if( SVCHK('a',0) ) {
            ...
            TreeNode* thisNode=(TreeNode*)SVO;
            XFuncNode* fnPrev=fn->xparent;
            sv=thisNode->gv("onInit");
            if( SVCHK('f',0) ) {                
                XFuncNode* fnParent=(XFuncNode*)SVO;
                if( fn!=fnParent ) fn->xparent=fnParent;
            }
            XFuncNode* fnCur = gfns.getFuncNode(fcCur, fn);
            if( fcCur ) {
                sv=arrs->get(2);
                if(SVCHK('a',0) ) {
                    XListArr* a=(XListArr*)SVO;
                    setFuncSrcParam(fnCur,a,fsrc,0,fn);
                } else {
                    arrs->remove(0,2);
                    setFuncSrcParam(fnCur,arrs,fsrc,0,fn);
                }
...
bool callEvalFunc(StrVar* sv, int sp, int ep, XFuncNode* fn, StrVar* rst, TreeNode* thisNode, StrVar* appendVar, bool bset ) {
    ...
    if( rst ) rst->reuse();
    qDebug("call eval func %d, %d",sp,ep);
	src->parseData(pv.GetVar(), sp, ep);
    if( src->makeFunc() && src->xfunc ) {
        ...
        if( func->getFuncSize()==1 ) {
			...
			qDebug("call eval func (type==%d)", fc->xftype);
		...
		*/
		XFuncNode* fnCur=gfns.getFuncNode(func, fn);
        if( thisNode==NULL ) {
            if( fn ) {
                StrVar* sv=fn->gv("@this");
                if( SVCHK('n',0) ) {
        ...
        if( appendVar ) {
            int sp=0,ep=0;
            sv=getStrVarPointer(appendVar,&sp,&ep);
            fnCur->GetVar("@appendVar")->setVar('s',0,(LPVOID)sv).addInt(sp).addInt(ep).addInt(sp).addInt(ep);
        }
        fnCur->call(func, rst );
        if( bset && fn ) {
            for( WBoxNode* bn=fnCur->First(); bn; bn=fnCur->Next() ) {
                LPCC varNm=fnCur->getCurCode();
                qDebug("eval bset var name=%s", varNm);
                if( varNm && varNm[0]!='@' ) {
                    sv=fn->GetVar(varNm);
		...
		qDebug("#9#eval function call match error");
	}
    return false;
}
