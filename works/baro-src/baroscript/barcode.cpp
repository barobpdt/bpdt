...
void QZint::render(QPainter & painter, const QRectF & paintRect, AspectRatioMode mode)
{
    ...
    // << "barcode render caption: "<<caption <<" error: "<< m_lastError <<" rect:"<<paintRect << " m_zintSymbol->symbology:" <<m_zintSymbol->symbology;
    if (m_lastError.length())
    {
        qDebug("#9#[barcode] render error:%s", Q2A(m_lastError));
