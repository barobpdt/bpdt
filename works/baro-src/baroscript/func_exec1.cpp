                ...
                if( cc || (func->xdotPos>0 && nchk==1) ) {
                    ...
                    if( cc==false ) {
                        ...
                        if( varNameCode[0]=='&' ) {
                            ...
                            if( func->xdotPos>0 && varNameCode[0]=='@' ) {
                                // qDebug("## may be subfunc %s\n",varNameCode );
                            } else {
                                if(ccmp(funcName,"val")) {
                                    sv=fn->GetVar(varNameCode);
                                    if(sv && sv->length()==0 ) {
                                        arrs=getParamArray(func,fn,rst);
                                        if(arrs) {
                                            for(int x=0,sz=arrs->size(); x<sz; x++) {
                                                sv->add(toString(arrs->get(x)));
                                            }
                                        }
                                        releaseLocalArray(arrs);
                                        return frtn;
                        ...
                        if( funcNm[0]=='&' ) {
                            ...
                            if( thisNode && SVCHK('f',1) ) {
                                ...
                                if( pfc ) {
                                    ...
                                    */
                                    XFuncNode* fnCur = gfns.getFuncNode(pfc, fnParent);
    ...
    */
    if( fnCur ) {
        ...
        if( arrs->xflag&FLAG_LOCALARRAY_SET ) {
            ...
            qDebug("xxxxxxxxxxx user func local array not set xxxxxxxxxxxxxx\n\n");
        }
        // releaseLocalArray(arrs);
        return true;
    }
