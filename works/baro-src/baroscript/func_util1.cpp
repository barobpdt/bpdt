...
TreeNode*	gTreeNodes = new TreeNode[64];
XListArr*	gArrays = new XListArr[64];
XListArr	gStrVars;
...
XFuncSrc*	gFuncSrcs = new XFuncSrc[8];
int gFuncSrcIndex=0;
...
StrVar* getStrVarTemp() {
    ...
    if( gStrVars.size()==0 ) {
        for(int n=0; n<256;n++ ) gStrVars.add();
    }
...
XListArr* getLocalArray(bool bset ) {
    ...
    while( true ) {
        ...
        arr=&gParams[gParamIndex++];
        if( arr->xflag & FLAG_LOCALARRAY_SET ) {
            qDebug("#0#getLocalArray alread set : %d, %d \n", start, gParamIndex );
            continue;
        }
        if( bset ) {
            arr->xflag|=FLAG_LOCALARRAY_SET;
...
bool callFuncSrc(XFuncSrc* fsrc, XFuncNode* fn, bool remove, TreeNode* thisNode, TreeNode* meNode, LPCC eventNodeName, PARR arrs, StrVar* rst ) {
    ...
    */
    ...
    sv=fnCur->gv("@echo");
    qDebug("echo size:%d fnm:%s\n", sv? sv->length():0, getCf()->get("@currentFuncName") );
    if( rst && sv && sv->length() ) {
        if( rst!=sv ) {
            rst->reuse()->add(sv);
        }
...
int setFuncSource(StrVar* var, int sp, int ep, LPCC funcLine, bool bset, bool bevent) {
    ...
    LPCC funcGroup=ref; //, funcName=NULL;
    if( c=='.' ) {
        pv.incr();
    ...
    qDebug("#0# function add %s (ch:%c)\n", funcNm, c);
    if( c=='{' ) {
        StrVar* sv=NULL;
        // ex) call("my.func() { print() }");
        while( pv.valid() ) {
            if( c!='{' ) break;
            ...
            */
            ...
            qDebug("#0#function add next... %s (%s ch:%c)\n", funcNm, param, c);
        }
    } else {
...
StrVar* getSubFuncSrc(LPCC funcNm, LPCC val, bool sqliteUse ) {
    LPCC funcCode=NULL;
    if( slen(val) ) {
        funcCode=gBuf.fmt("%s::%s",funcNm, val);
    } else {
        if( slen(funcNm)==0 ) {
            return NULL;
        }
    ...
    return getStrVar("subfunc", funcCode);
    /*
    if( SVCHK('f',1) ) {
        return sv;
    } else if( sv ) {
    ...
    if( sqliteUse ) {
        return getSubFuncSrcSqlite(funcCode );
    }
    SqlDatabase* db=getModelDatabase("pages");
    if( !db->isOpen() ) {
        TreeNode* tn=db->xcf;
        qDebug("#0#getSubfuncSrc database open retry (funcname:%s)\n", funcCode);
        LPCC dsn= tn->get("dsn");
        LPCC inode = tn->get("inode");
        if( tn && !db->open(tn->get(slen(inode)?inode:dsn)) ) {
            return NULL;
        }
    }
    if( db->prepare("select funcname, funcparam, funcdata, funcline from subFunc where funcRef=? and funcName=? ") ) {
        ...
        db->bindStr(0,funcNm).bindStr(1,val).exec();
        if( db->fetchNode() ) {
            XFuncSrc* fsrc = gfsrcs.getFuncSrc();
            StrVar* funcData=db->GetVar("funcdata");
            fsrc->readBuffer(funcData,0,funcData->length() );
            if( fsrc->makeFunc() && fsrc->xfunc ) {
                ...
                if( ccmp(funcLine,"interface") ) {
                    fsrc->xflag|=0x400;
                } else if( ccmp(funcLine,"event") || ccmp(funcLine,"common") ) {
                    fsrc->xflag|=0x800 | 0x400;
                } else if( ccmp(funcLine,"sub") ) {
                    fsrc->xflag|=0x800;
                }
                ...
                gMutexSubFunc.EnterMutex();
                getStrVar("subfunc", funcCode, 'f',1,(LPVOID)fsrc );
                gMutexSubFunc.LeaveMutex();
                return sv;
            } else {
                gfsrcs.deleteFuncSrc(fsrc);
...
bool isTrueCheck(XFunc* fc) {
    ...
    if( bchk ) {
        ...
        if( pfc && (ftype==FTYPE_CONTROL || ftype==FTYPE_FUNCCHECK || pfc->getParam(0)->getType()==FTYPE_OPER ) ) {
